...
const Sidebar: React.FC<SidebarProps> = ({ user, onProfileClick, onRankingClick }) => {
  const menuItems = [
    { icon: 'fa-chess-board', label: 'Jogar', active: true },
    { icon: 'fa-trophy', label: 'Ranking', onClick: onRankingClick },
      ...
      <nav className="flex-1">
        {menuItems.map((item, idx) => (
          <button 
            ...
            className={`w-full flex flex-col lg:flex-row items-center lg:space-x-4 px-2 lg:px-6 py-4 hover:bg-[#3c3a37] transition-colors ${item.active ? 'nav-active' : ''}`}
          >
            <i className={`fas ${item.icon} text-xl ${item.active ? 'text-[#81b64c]' : 'text-gray-400'}`}></i>
